...
local tModKeys = {[vkeys.VK_MENU] = true, [vkeys.VK_SHIFT] = true, [vkeys.VK_CONTROL] = true, [vkeys.VK_RBUTTON] = true, [vkeys.VK_LBUTTON] = true,}
local tBlockNext = {}
local module = {}
...
module.HotKey = function(name, keys, width, callback)
   ...
   local name = tostring(name)
    ...
    if imgui.Button((tostring(sKeys):len() == 0 and u8(module._SETTINGS.noKeysMessage) or u8(sKeys)) .. name, imgui.ImVec2(width, 0)) then
        edit = name
    end
...
addEventHandler("onWindowMessage", function (msg, wparam, lparam)
	...
	if keysDown[msg] then
      ...
      if msg == wm.WM_LBUTTONDOWN then wparam = 1 end
		-- if msg == wm.WM_XBUTTONDOWN then wparam = bit.rshift(bit.band(wparam, 0xffff0000), 16) end
	...
	elseif keysUp[msg] then 
      ...
      if msg == wm.WM_LBUTTONUP then wparam = 1 end
		-- if msg == wm.WM_XBUTTONUP then wparam = bit.rshift(bit.band(wparam, 0xffff0000), 16) end
